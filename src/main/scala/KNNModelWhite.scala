...
object KNNModelWhite {
  def main(args: Array[String]): Unit = {
    ...
    // Importing Dataset
    val dataRaw = sc.textFile("winequality-white.csv")
   ...
   // })
      ...
      val parsed = data.map(fields => {
      // Drop features 0, 2, 3, 6, 7, 8, 9, 10
      val selectedIndices = Array( 1, 4, 5) // features to keep
      val features = selectedIndices.map(i => fields(i).toDouble)
      val quality = fields(11).toDouble
      (features, quality)
   })
